	...
	Context("based on old syslog plugin", func() {
		...
		Context("for protocol-less endpoint", func() {
			...
			It("should produce well formed output label config", func() {
				...
				Expect(results[0]).To(MatchLines(tcpConf))
			})
		})
		...
		Context("for udp endpoint", func() {
			...
			It("should produce well formed output label config", func() {
				...
				Expect(err).To(BeNil())
				Expect(len(results)).To(Equal(1))
				Expect(results[0]).To(MatchLines(udpConf))
			})
		})
	})
		...
		Context("for tcp endpoint", func() {
			...
			Context("with TLS enabled", func() {
				...
				It("should produce well formed output label config", func() {
					...
					Expect(err).To(BeNil())
					Expect(len(results)).To(Equal(1))
					Expect(results[0]).To(MatchLines(tcpWithTLSConf))
				})
			})
		})
		...
		Context("for udp endpoint", func() {
			...
			Context("with TLS enabled", func() {
				...
				It("should produce well formed output label config", func() {
					...
					Expect(err).To(BeNil())
					Expect(len(results)).To(Equal(1))
					Expect(results[0]).To(MatchLines(udpWithTLSConf))
				})
			})
		})
	})
