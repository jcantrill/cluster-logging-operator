...
// ContainerInput is a source of logs generated by containers running on a kubernetes cluster
...
func NewContainerSource(spec ContainerInput, includes, excludes string, logType obs.InputType, logSource interface{}) ([]framework.Element, []string) {
	...
	el := []framework.Element{
		source.KubernetesLogs{
			...
			ExtraLabelSelector: source.LabelSelectorFrom(spec.PodSelector()),
		},
		NewInternalNormalization(metaID, logSource, logType, base),
	}
	inputID := metaID
	//TODO: DETERMINE IF key field is correct and actually works
	if threshold, hasPolicy := internalobs.MaxRecordsPerSecond(spec.RateLimitPerContainer()); hasPolicy {
